...
# The entry point
#
def nmosHV(cv, w=0.15e-6, fw=0.15e-6, l=0.45e-6, ng=1, calculate=["finger_width", "total_width"], gate_contacts=["none", "bottom"], body_tie=["none", "left", "left&right", "top"], drain_metal=["m1", "m2", "m3"], source_metal=["m1", "m2", "m3"], bulk_metal=["m1", "m2", "m3"]) :
    lib = cv.lib()
    tech = lib.tech()
    dbu = lib.dbuPerUU()
    ...
    if (body_tie == "top") :
        ...
        yp1 = int(yp0 + 2 * activ_en_cont + cont_width + 2 * psd_en_activ)
        r = Rect(xp0, yp0, xp1, yp1)
        psd = cv.dbCreateRect(r, layer)
        layer = tech.getLayerNum("Activ", "drawing")
        ...
        if (bulk_metal == "m3") :
            ...
            for n in range(n_via) :
                ...
                r = Rect(xv0, yv0, xv1, yv1)
                via2 = cv.dbCreateRect(r, layer)
    # Create Thick Gate Oxide layer
    layer = tech.getLayerNum("ThickGateOx", "drawing")
    xtgo0 = int(-(cont_activ_to_gatpoly + cont_width + activ_en_cont + thickgateox_ex_activ))
    ytgo0 = int(-(activ_ex_offset + thickgateox_ex_activ))
    xtgo1 = int(n_fingers * (length + gate_offset) - cont_activ_to_gatpoly + activ_en_cont + thickgateox_ex_activ)
    ytgo1 = int(width + activ_ex_offset + thickgateox_ex_activ)
    if (width < (2 * activ_en_cont + cont_width)) :
        xtgo0 = int(-(gate_offset - gate_to_activ + thickgateox_ex_activ))
        xtgo1 = int(n_fingers * (length + gate_offset) - gate_to_activ + thickgateox_ex_activ)
    if ((body_tie == "left") or (body_tie == "left&right")) :
        xtgo0 = int(-(y * cont_activ_to_gatpoly + cont_width + cont_offset + x * activ_en_cont + metal1_space + metal1_width + activ_en_cont + thickgateox_ex_activ))
    if (body_tie == "left&right") :
        xtgo1 = int(xa1 + thickgateox_ex_activ)
    if (body_tie == "top") :
        ytgo1 = int(ya1 + thickgateox_ex_activ)
    if (gate_contacts == "bottom") :
        ytgo0 = int(-(metal1_space + 2 * activ_ex_offset + metal1_en_cont + cont_width + gatepoly_en_cont + thickgateox_ex_activ - gatepoly_ex_activ))
