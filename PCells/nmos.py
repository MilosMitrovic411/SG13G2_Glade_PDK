...
# The entry point
#
def nmos(cv, w=0.15e-6, l=0.13e-6, ng=1, gate_contacts=["none", "bottom"], body_tie=["none", "left", "left&right", "top"], drain_metal=["m1", "m2", "m3"], source_metal=["m1", "m2", "m3"], bulk_metal=["m1", "m2", "m3"]) :
    lib = cv.lib()
    tech = lib.tech()
    dbu = lib.dbuPerUU()
    width = int(w * 1e9)
