...
# The entry point
#
def diodevss_4k(cv, type="dio", model = "diodevss_4kv") :
    lib = cv.lib()
    tech = lib.tech()
    dbu = lib.dbuPerUU()
    #
    ...
    r = Rect(xa0, ya0, xa1, ya1)
    activ = cv.dbCreateRect(r, activ_layer)
    xp0 = int(xn0 + 4.53 * dbu)
    yp0 = int(yn0 + 2.1 * dbu)
    xp1 = int(xn0 + 6.63 * dbu)
    yp1 = int(yn1 - 2.1 * dbu)
    ...
    for n in range(n_cont) :
        ...
        r = Rect(xc0, yc0, xc1, yc1)
        cont = cv.dbCreateRect(r, cont_layer)
    p = Point(int(xn0 + 5.615 * dbu), int(yn1 + 0.87 * dbu))
    ...
    p = Point(int(xn0 + 1.59 * dbu), int(yn0 + 0.815 * dbu))
