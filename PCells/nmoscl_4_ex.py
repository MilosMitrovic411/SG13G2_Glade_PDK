...
from ui import *
#
def nmoscl_4_ex(cv, ptlist=[[0,0],[1000,0],[1000,1000],[0,1000]]) :
    lib = cv.lib()
    ...
    cv.dbAddProp("model", "nmoscl_4")

    # Set the NLP property for netlisting
    cv.dbAddProp("NLPDeviceFormat", "c[@instName] [|VDD:%] [|VSS:%] [@model] [@m:m=%]")
    cv.dbAddProp("NLPDeviceFormatCDL", "x[@instName] [|VDD:%] [|VSS:%] [@model] [@m:m=%]")
