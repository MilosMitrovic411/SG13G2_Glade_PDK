...
from ui import *
#
def npn13G2_ex(cv, ptlist=[[0,0],[1000,0],[1000,1000],[0,1000]]) :
    lib = cv.lib()
    ...
    cv.dbAddProp("modelName", "npn13G2")

    # Set the NLP property for netlisting
    cv.dbAddProp("NLPDeviceFormat", "[@instName] [|C:%] [|B:%] [|E:%] [|sub!:%] [@modelName] le=0.9e-6 we=0.07e-6")
    cv.dbAddProp("NLPDeviceFormatCDL", "x[@instName] [|C:%] [|B:%] [|E:%] [|sub!:%] [@modelName] le=0.9e-6 we=0.07e-6")
