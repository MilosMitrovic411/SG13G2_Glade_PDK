...
# The entry point
#
def npn13G2V(cv, emitterLength=1e-6, numberOfEmitters=1, type="bjt", modelName="npn13G2v") :
    lib = cv.lib()
    tech = lib.tech()
    dbu = lib.dbuPerUU()
    ...
    for i in range(ne):
        ...
        xc0 = int(xe0 - 0.02 * dbu)
        yc0 = int(ye0 - 0.06 * dbu)
        xc1 = int(xe1 + 0.02 * dbu)
        yc1 = int(ye1 + 0.06 * dbu)
        r = Rect(xc0, yc0, xc1, yc1)
        cont = cv.dbCreateRect(r, cont_layer)
        xm10 = int(xc0 - 0.05 * dbu)
        ym10 = int(yc0 - 0.22 * dbu)
        xm11 = int(xc1 + 0.05 * dbu)
        ym11 = int(yc1 + 0.22 * dbu)
        r = Rect(xm10, ym10, xm11, ym11)
        metal1 = cv.dbCreateRect(r, metal1_layer)
        activ = cv.dbCreateRect(r, activ_layer)
        n_via1 = int((ym10 - ym11 - 2 * metal_en_via + via_space) / (via_width + via_space))
        for n in range(n_via1) :
            xv10 = int(xm10 + 0.035 * dbu)
            yv10 = int(ym10 + metal_en_via + n * (via_width + via_space))
            xv11 = int(xc0 + cont_width)
            yv11 = int(yc0 + cont_width)
            r = Rect(xv10, yv10, xv11, yv11)
            via1 = cv.dbCreateRect(r, via1_layer)
        #
        xa0 = int(-1.11 * dbu)
        ya0 = ym10
        xa1 = int(xa0 + 0.405 * dbu)
        ya1 = ym11
        r = Rect(xa0, ya0, xa1, ya1)
        activ = cv.dbCreateRect(r, activ_layer)
        xm10 = xa0
        ym10 = ya0
        xm11 = int(xm10 + 0.32 * dbu)
        ym11 = int(ya1 + 0.72 * dbu)
        r = Rect(xm10, ym10, xm11, ym11)
        metal1 = cv.dbCreateRect(r, metal1_layer)
        n_cont = int((ya1 - ya0 - 2 * activ_en_cont + cont_space) / (cont_width + cont_space))
        for n in range(n_cont) :
            xcl0 = int(xa0 + 0.1 * dbu)
            ycl0 = int(ya0 + activ_en_cont + n * (cont_width + cont_space))
            xcl1 = int(xcl0 + cont_width)
            ycl1 = int(ycl0 + cont_width)
            r = Rect(xcl0, ycl0, xcl1, ycl1)
            cont = cv.dbCreateRect(r, cont_layer)
        #
        xam0 = xa1
        yam0 = ya0
        xam1 = int(xam0 + 0.635 * dbu)
        yam1 = ya1
        r = Rect(xam0, yam0, xam1, yam1)
        activ_mask = cv.dbCreateRect(r, activ_mask_layer)
        for n in range(n_cont) :
            xc0 = int(xam0 + 0.245 * dbu)
            yc0 = int(yam0 + activ_en_cont + n * (cont_width + cont_space))
            xc1 = int(xc0 + cont_width)
            yc1 = int(yc0 + cont_width)
            r = Rect(xc0, yc0, xc1, yc1)
            cont = cv.dbCreateRect(r, cont_layer)
        xm10 = int(xc0 - 0.005 * dbu)
        ym10 = int(yam0 - 0.72 * dbu)
        xm11 = int(xc1 + 0.005 * dbu)
        ym11 = yam1
        r = Rect(xm10, ym10, xm11, ym11)
        metal1 = cv.dbCreateRect(r, metal1_layer)
        #
        ...
        metal1 = cv.dbCreateRect(r, metal1_layer)
        for n in range(n_cont) :
            xcr0 = int(xa0 + 0.145 * dbu)
            ycr0 = int(ya0 + activ_en_cont + n * (cont_width + cont_space))
            xcr1 = int(xcr0 + cont_width)
            ycr1 = int(ycr0 + cont_width)
            r = Rect(xcr0, ycr0, xcr1, ycr1)
            cont = cv.dbCreateRect(r, cont_layer)
        #
        xam0 = int(xa0 - 0.635 * dbu)
        yam0 = ya0
        xam1 = xa0
        yam1 = ya1
        r = Rect(xam0, yam0, xam1, yam1)
    ...
    p = Point(int((xm21 - xm20) / 2 - 1.11 * dbu), int((ym21 - ym20) / 2 - 0.2 * dbu))
    text = cv.dbCreateLabel(p, "E", R0, 0.7, centreCentre, text_layer)
    #
    xm10 = xm20
    ym10 = int(ym21 + 0.72 * dbu)
    xm11 = xm21
    ...
    p = Point(int((xm11 - xm10) / 2 - 1.11 * dbu), int((ym11 - ym10) / 2 + l + 1 * dbu))
    text = cv.dbCreateLabel(p, "C", R0, 0.3, centreCentre, text_layer)
    #
    xm10 = int(xm20 + 0.645 * dbu)
    ym10 = int(ym20 - 0.72 * dbu)
    xm11 = int(xm21 - 0.645 * dbu)
    ym11 = int(ym10 - 0.65 * dbu)
    r = Rect(xm10, ym10, xm11, ym11)
    metal1 = cv.dbCreateRect(r, metal1_layer)
    ...
    cv.dbCreatePort(pin, metal1)
    p = Point(int((xm11 - xm10) / 2 - 0.465 * dbu), int((ym11 - ym10) / 2 - 1 * dbu))
    text = cv.dbCreateLabel(p, "B", R0, 0.3, centreCentre, text_layer)
    #
    xt0 = int(xm20 - 1.8 * dbu)
    yt0 = int(ym20 - 1.92 * dbu)
    xt1 = int(xm21 + 1.8 * dbu)
    yt1 = int(ym21 + 1.92 * dbu)
    ...
    text = cv.dbCreateLabel(p, "npn13G2V", R0, 0.37, bottomLeft, text_layer)
    str_ne = str(ne)
    float_l = float(l / dbu)
    str_l = str(float_l)
    string = "Ae = " + str_ne + " * " + str_l + " * 0.12"
