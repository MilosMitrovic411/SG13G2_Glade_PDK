...
def SVaricap_ex(cv, ptlist=[[0,0],[1000,0],[1000,1000],[0,1000]]) :
    lib = cv.lib()
    ...
    # Create pins
    top_net = cv.dbCreateNet("G1")
    cv.dbCreatePin("G1", top_net, DB_PIN_INPUT)
    bot_net = cv.dbCreateNet("W")
    cv.dbCreatePin("W", bot_net, DB_PIN_INPUT)
    top_net = cv.dbCreateNet("G2")
    cv.dbCreatePin("G", top_net, DB_PIN_INPUT)
    top_net = cv.dbCreateNet("sub!")
    cv.dbCreatePin("sub!", top_net, DB_PIN_INPUT)
    
    # Setting device type to capacitor
    cv.dbAddProp("type", "cap")

    # Set the device modelName property for netlisting
    cv.dbAddProp("model", "sg13_hv_svaricap")

    # Set the NLP property for netlisting
    cv.dbAddProp("NLPDeviceFormat", "c[@instName] [|G1:%] [|W:%] [|G2:%] [|sub!:%] [@model] [@w:w=%u] [@l:l=%u] [@Nx:Nx=%]")
    cv.dbAddProp("NLPDeviceFormatCDL", "x[@instName] [|G1:%] [|W:%] [|G2:%] [|sub!:%] [@model] [@w:w=%u] [@l:l=%u] [@Nx:Nx=%]")
