...
# The entry point
#
def diodevss_2k(cv, type="dio", model = "diodevss_2kv") :
    lib = cv.lib()
    tech = lib.tech()
    dbu = lib.dbuPerUU()
    #
    ...
    # Creating Anode
    #
    xn0 = int(xa0 - 2.67 * dbu)
    yn0 = int(ya0 - 3.17 * dbu)
    xn1 = int(xa1 + 2.7 * dbu)
    ...
    psd_path = cv.dbCreatePath(xap, yap, 5, psd_layer, int(2.1 * dbu), 2, int(1.05 * dbu), int(1.05 * dbu))
    n_points = 10
    ...
    nwell = cv.dbCreatePolygon(xm, ym, n_points, nwell_layer)
    n_cont = int((8.88 * dbu  - 2 * activ_en_cont + cont_space) / (cont_width + cont_space))
    ...
    for n in range(n_cont) :
        ...
        r = Rect(xc0, yc0, xc1, yc1)
        cont = cv.dbCreateRect(r, cont_layer)
    p = Point(int(xn0 + 3.315 * dbu), int(yn1 + 0.87 * dbu))
    text = cv.dbCreateLabel(p, "VDD", R0, 0.2,  centreCentre, text_layer)
    #
    ...
    text = cv.dbCreateLabel(p, "VSS", R0, 0.2,  bottomLeft, text_layer)
    p = Point(int(xn0+ 0.43 * dbu), int(yn0 + 0.7 * dbu))
