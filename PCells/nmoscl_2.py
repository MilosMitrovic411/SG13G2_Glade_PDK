...
# The entry point
#
def nmoscl_2(cv, type="dio", model = "nmoscl_2") :
    lib = cv.lib()
    tech = lib.tech()
    dbu = lib.dbuPerUU()
    #
    ...
    # Creating Bulk
    #
    xac0 = int(xa0 + 0.005 * dbu)
    yac0 = int(ya1 + 1.5 * dbu)
    xac1 = int(xac0 + 32.71 * dbu)
    yac1 = int(yac0 + 0.63 * dbu)
    r = Rect(xac0, yac0, xac1, yac1)
    activ = cv.dbCreateRect(r, activ_layer)
    p = Point(int(xac0 + 1.15 * dbu), int(yac0 + 0.18 * dbu))
    text = cv.dbCreateLabel(p, "sub!", R0, 0.2,  bottomLeft, text_layer)
    xp0 = int(xac0 - 0.33 * dbu)
    yp0 = int(yac0 - 0.36 * dbu)
    xp1 = int(xp0 + 33.37 * dbu)
    yp1 = int(yp0 + 1.32 * dbu)
    r = Rect(xp0, yp0, xp1, yp1)
    psd = cv.dbCreateRect(r, psd_layer)
    n_cont = int((32 * dbu - 2 * activ_en_cont + cont_space ) / (cont_width + cont_space))
    for n in range(n_cont) :
        xc0 = int(xac0 + 0.47 * dbu + n * (cont_width + cont_space))
        yc0 = int(yac0 + 0.235 * dbu)
        xc1 = int(xc0 + cont_width)
        yc1 = int(yc0 + cont_width)
        r = Rect(xc0, yc0, xc1, yc1)
        cont = cv.dbCreateRect(r, cont_layer)
    ...
    xap = intarray(5)
    yap = intarray(5)
    xap[0] = int(xa0 - 1.51 * dbu)
    yap[0] = int(ya0 - 2.415 * dbu)
    xap[1] = int(xa1 + 1.65 * dbu)
    yap[1] = yap[0]
    xap[2] = xap[1]
    yap[2] = int(ya1 + 3.675 * dbu)
    xap[3] = xap[0]
    yap[3] = yap[2]
    xap[4] = xap[0]
    yap[4] = yap[0]
    activ_path = cv.dbCreatePath(xap, yap, 5, activ_layer, int(0.63 * dbu), 2, int(0.315 * dbu), int(0.315 * dbu))
    n_points = 10
    ...
    xm3[48] = xm3[47]
    ym3[48] = ym3[4]
    xm3[49] = int(xm3[48] - 0.72 * dbu)
    ym3[49] = ym3[4]
    xm3[50] = xm3[49]
    ym3[50] = ym3[2]
    xm3[51] = int(xm3[50] - 1.05 * dbu)
    ym3[51] = ym3[2]
    ...
    n_cont = int((34.5 * dbu - 2 * activ_en_cont + cont_space + 0.02 * dbu) / (cont_width + cont_space + 0.02 * dbu))
    for n in range(n_cont) :
        xc0 = int(xa0 - 0.705 * dbu + n * (cont_width + cont_space + 0.02 * dbu))
        yc0 = int(ya0 - 2.515 * dbu)
        xc1 = int(xc0 + cont_width)
        yc1 = int(yc0 + cont_width)
        r = Rect(xc0, yc0, xc1, yc1)
        cont = cv.dbCreateRect(r, cont_layer)
        yc0 = int(ya1 + 3.575 * dbu)
        yc1 = int(yc0 + cont_width)
        r = Rect(xc0, yc0, xc1, yc1)
        cont = cv.dbCreateRect(r, cont_layer)
    n_cont = int((20.46 * dbu - 2 * activ_en_cont + cont_space ) / (cont_width + cont_space))
    for n in range(n_cont) :
        xc0 = int(xa0 - 1.585 * dbu)
        yc0 = int(ya0 - 2.55 * dbu + n * (cont_width + cont_space))
        xc1 = int(xc0 + cont_width)
        yc1 = int(yc0 + cont_width)
        r = Rect(xc0, yc0, xc1, yc1)
        cont = cv.dbCreateRect(r, cont_layer)
        xc0 = int(xa1 + 1.595 * dbu)
        xc1 = int(xc0 + cont_width)
        r = Rect(xc0, yc0, xc1, yc1)
        cont = cv.dbCreateRect(r, cont_layer)
    n_via1 = int((35 * dbu - 2 * metal_en_via + via_space ) / (via_width + via_space))
    for n in range(n_via1) :
        xv0 = int(xa0 - 1.17 * dbu + n * (via_width + via_space))
        yv0 = int(ya0 - 2.525 * dbu)
        xv1 = int(xv0 + via_width)
        yv1 = int(yv0 + via_width)
        r = Rect(xv0, yv0, xv1, yv1)
        via1 = cv.dbCreateRect(r, via1_layer)
        yv0 = int(ya1 + 3.565 * dbu)
        yv1 = int(yv0 + via_width)
        r = Rect(xv0, yv0, xv1, yv1)
        via1 = cv.dbCreateRect(r, via1_layer)
    n_via1 = int((20.01 * dbu - 2 * metal_en_via + via_space ) / (via_width + via_space))
    for n in range(n_via1) :
        xv0 = int(xa0 - 1.6 * dbu)
    ...
    p = Point(int(xa0 + 1.32 * dbu), int(ya1 + 3.93 * dbu))
    text = cv.dbCreateLabel(p, "VDD", R0, 0.2,  bottomLeft, text_layer)
    p = Point(int(xa0 + 1.545 * dbu), int(ya0 - 2.355 * dbu))
