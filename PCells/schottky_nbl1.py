...
def schottky_nbl1(cv) :
    ...
    ympp[0] = int(ys0 - (salblock_to_cont + cont_width + activ_en_cont)/ 2)
    xmpp[1] = int(xs1 + activ_width / 2)
    ympp[1] = ympp[0]
    a_path = cv.dbCreatePath(xmpp, ympp, 2, a_layer, int(salblock_to_cont + cont_width + activ_en_cont), 2, int(activ_width / 2), int(activ_width / 2))
    xmpp = intarray(2)
    ympp = intarray(2)
    xmpp[0] = int(xs0 - activ_width / 2)
    ympp[0] = int(ys0 - salblock_to_cont - cont_width / 2)
    xmpp[1] = int(xs1 + activ_width / 2)
    ympp[1] = ympp[0]
    ...
    ympp[0] = int(ys0 - salblock_to_cont - cont_width / 2)
    xmpp[1] = xmpp[0]
    ympp[1] = int(ys1 + salblock_to_cont + cont_width / 2)
    ...
    ympp = intarray(2)
    xmpp[0] = int(xs0 - activ_width / 2)
    ympp[0] = int(ys1 + (salblock_to_cont + cont_width + activ_en_cont)/ 2)
    xmpp[1] = int(xs1 + activ_width / 2)
    ympp[1] = ympp[0]
    a_path = cv.dbCreatePath(xmpp, ympp, 2, a_layer, int(salblock_to_cont + cont_width + activ_en_cont), 2, int(activ_width / 2), int(activ_width / 2))
    xmpp = intarray(2)
    ympp = intarray(2)
    xmpp[0] = int(xs0 - activ_width / 2)
    ympp[0] = int(ys1 + salblock_to_cont + cont_width / 2)
    xmpp[1] = int(xs1 + activ_width / 2)
    ympp[1] = ympp[0]
    m_path = cv.dbCreatePath(xmpp, ympp, 2, m_layer, int(2 * metal1_en_cont+ cont_width), 0, 0, 0)
    net = cv.dbCreateNet("C")
    ...
    for n in range (n_cont) :
        xc0 = int(xs0 - activ_width + activ_en_cont + offset + n * (cont_width + cont_space))
        yc0 = int(ys1 + salblock_to_cont)
        xc1 = int(xc0 + cont_width)
        yc1 = int(yc0 + cont_width)
        r = Rect(xc0, yc0, xc1, yc1)
        cont = cv.dbCreateRect(r, layer)
    layer = tech.getLayerNum("NWell", "drawing")
    xn0 = int(xs0 - activ_width - nwell_en_nsd_activ)
    yn0 = int(ys0 - salblock_to_cont - cont_width - activ_en_cont - nwell_en_nsd_activ)
    xn1 = int(xs1 + activ_width + nwell_en_nsd_activ)
    yn1 = int(ys1 + salblock_to_cont + cont_width + activ_en_cont + nwell_en_nsd_activ)
    ...
    r = Rect(int(xn0 + activ_width), int(yn0 + activ_width), int(xn1 - activ_width), int(yn1 - activ_width))
