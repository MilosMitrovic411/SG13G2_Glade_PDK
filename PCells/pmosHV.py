...
# The entry point
#
def pmosHV(cv, w=0.15e-6, fw=0.15e-6, l=0.4e-6, ng=1, calculate=["finger_width", "total_width"], gate_contacts=["none", "bottom"], body_tie=["none", "left", "left&right", "top"], drain_metal=["m1", "m2", "m3"], source_metal=["m1", "m2", "m3"], bulk_metal=["m1", "m2", "m3"]) :
    lib = cv.lib()
    tech = lib.tech()
    dbu = lib.dbuPerUU()
    ...
    nwell_en_psd_activ = int(0.62 * dbu)
    nwell_en_nsd_activ = int(0.62 * dbu)
    activ_width = int(0.15 * dbu)
    activ_space = int(0.21 * dbu)
    activ_ex_ds = int(0.23 * dbu)
    gatepoly_width = int(0.4 * dbu)
    gatepoly_ex_activ = int(0.18 * dbu)
    gate_to_activ = int(0.07 * dbu)
    thickgateox_ex_activ = int(0.27 * dbu)
    thickgateox_to_activ = int(0.27 * dbu)
    thickgateox_ex_gatepoly_activ = int(0.34 * dbu)
    thickgateox_width = int(0.86 * dbu)
    ...
    if ((body_tie == "left") or (body_tie == "left&right")) :
        ...
        for n in range(n_cont) :
            ...
            cont = cv.dbCreateRect(r, layer)
        layer = tech.getLayerNum("Activ", "drawing")
        xa0 = int(xc0 - activ_en_cont)
        ya0 = int(ym0)
        xa1 = int(-(gate_offset - cont_activ_to_gatpoly + activ_en_cont))
        ya1 = int(ym1)
        if (width < (2 * activ_en_cont + cont_width)) :
            xa1 = int(- gate_offset + gate_to_activ)
        r = Rect(xa0, ya0, xa1, ya1)
        activ = cv.dbCreateRect(r, layer)
        cv.dbCreatePort(pin, metal1)
    ...
    xn0 = int(-(cont_activ_to_gatpoly + cont_width + activ_en_cont + nwell_en_psd_activ))
    yn0 = int(-(nwell_en_psd_activ + activ_ex_offset))
    xn1 = int(n_fingers * (length + gate_offset) - gate_offset + cont_activ_to_gatpoly + cont_width + activ_en_cont + nwell_en_psd_activ)
    yn1 = int(width + activ_ex_offset + nwell_en_psd_activ)
    if (width < (2 * activ_en_cont + cont_width)) :
        xn0 = int(-(gate_offset - gate_to_activ + nwell_en_psd_activ))
        xn1 = int(n_fingers * (length + gate_offset) - gate_to_activ + nwell_en_psd_activ)
    if (body_tie == "left") :
        xn0 = int(xa0 - nwell_en_psd_activ)
    if (body_tie == "left&right") :
        xn0 = int(-(y * cont_activ_to_gatpoly + cont_width + metal1_en_cont + cont_offset + x * activ_en_cont + metal1_space + metal1_width - metal1_en_cont + activ_en_cont + nwell_en_psd_activ))
        xn1 = int(xa1 + nwell_en_psd_activ)
    if (body_tie == "top") :
        yn1 = int(ya1 + nwell_en_nsd_activ)
    if (gate_contacts == "bottom") :
        yn0 = int(-(metal1_space + 2 * activ_ex_offset + metal1_en_cont + cont_width + gatepoly_en_cont + nwell_en_psd_activ - gatepoly_ex_activ))
    ...
    # Create Thick Gate Oxide
    layer = tech.getLayerNum("ThickGateOx", "drawing")
    xtgo0 = xn0
    ytgo0 = yn0
    xtgo1 = xn1
    ytgo1 = yn1
    r = Rect(xtgo0, ytgo0, xtgo1, ytgo1)
    thickgateox = cv.dbCreateRect(r, layer)
    #
    # Add device type
    #
