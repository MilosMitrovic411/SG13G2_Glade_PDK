...
# The entry point
#
def hv_inmos(cv, w=0.15e-6, l=0.13e-6, nf=1, gate_contacts=["none", "bottom"], body_tie=["left", "left&right"], drain_metal=["m1", "m2", "m3"], source_metal=["m1", "m2", "m3"], bulk_metal=["m1", "m2", "m3"]) :
    lib = cv.lib()
    tech = lib.tech()
    dbu = lib.dbuPerUU()
    width = int(w * 1e9)
    ...
    xtgo0 = int(xnbl0 + nwell_en_nsd_activ - thickgateox_ex_activ)
    ytgo0 = int(ynbl0 + nwell_en_nsd_activ - thickgateox_ex_activ)
    xtgo1 = int(xnbl1 - nwell_en_nsd_activ + thickgateox_ex_activ)
    ytgo1 = int(ynbl1 - nwell_en_nsd_activ + thickgateox_ex_activ)
